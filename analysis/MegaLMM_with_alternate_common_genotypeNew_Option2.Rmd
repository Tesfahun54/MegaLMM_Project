---
title: "MegaLMM analysis with alternate overlapping genotypes along trials"
...
Acc_MegaLMM_scenario_2 <- tibble()

for(pr in 1:10){
#Option 2- alternate common genotypes 
...
Y2 <- Y2[,c(Trial_min, Trial_max)]
head(Y2)
#order trial names 
Trial_ord <- c(Trial_min, Trial_max) #
#list name of accession for trial 1 and 2, trial 2 and 3
...
#List genotypes common across the three trials and btween trial 1 and 2

Int123 <- Reduce(intersect, list(Geno_Trial1, Geno_Trial2, Geno_Trial3))
Int12 <- Reduce(intersect, list(Geno_Trial1, Geno_Trial2))

#List accessions common for trial 1 and 2, trial 2 and 3
if(length(Int12) > 25 & length(Int123) < 25){
gencom_12 <- sample(intersect(Geno_Trial1,Geno_Trial2)[!intersect(Geno_Trial1,Geno_Trial2) %in% Int123],size = 20, replace = F)
gencom_23 <- sample(intersect(Geno_Trial2,Geno_Trial3)[!intersect(Geno_Trial2,Geno_Trial3) %in% gencom_12] ,size = 20, replace = F)
}else if(length(Int12) > 100){
  gencom_12 <- sample(intersect(Geno_Trial1,Geno_Trial2),size = 20, replace = F)
  gencom_23 <- sample(intersect(Geno_Trial2,Geno_Trial3)[!intersect(Geno_Trial2,Geno_Trial3) %in% gencom_12] ,size = 20, replace = F)
  }else{
  gencom_12 <- sample(intersect(Geno_Trial1,Geno_Trial2),size = length(Int123), replace = F)
  gencom_23 <- sample(intersect(Geno_Trial2,Geno_Trial3)[!intersect(Geno_Trial2,Geno_Trial3) %in% gencom_12] ,size = 20, replace = F)
}

#Select accessions for a training set for each trial

Train_geno1 <- c(gencom_12, sample(Geno_Trial1[!Geno_Trial1 %in% c(gencom_12,gencom_23)], size = 20))
Train_geno2 <- c(gencom_12,gencom_23, sample(Geno_Trial2[!Geno_Trial2 %in% c(gencom_12,gencom_23, Train_geno1)], size = 0))
Train_geno3 <- c(gencom_23, sample(Geno_Trial3[!Geno_Trial3 %in% c(gencom_12,gencom_23, Train_geno1, Train_geno2)], size = 20))

intersect(Train_geno1,Train_geno3)
...
Acc_MegaLMM_scenario_2 = rbind(Acc_MegaLMM_scenario_2, acc)

}

Acc_MegaLMM_scenario_2$Method<- "Scenario2"
Acc_MegaLMM_scenario_2$Group <- "Group6"
Acc_MegaLMM_scenario_2$plot_per_Trial <- 40
Acc_MegaLMM_scenario_2$Trait <- "Plant_height"
