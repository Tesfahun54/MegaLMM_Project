---
title: "MegaLMM analysis with common genotype across trials and additional alternate
  common genotypes"
...
################
# Selecting trials with genotypes > 100
geno_sum <- tibble()
for(i in unique(phen$studyName)){
  ph1 <- phen[phen$studyName == i, ]
  l <- length(unique(ph1$germplasmName))
 l1 <-  cbind(Trial = i, geno <- l)
 geno_sum <- rbind(geno_sum,l1)
}

idt <- which(as.numeric(geno_sum$V2) > 100)
Trial_100 <- geno_sum$Trial[idt]

phensel <- phen[phen$studyName %in% Trial_100,]
...
phensel$geno
 for(Trait in Traits){
  dt = phensel[,c("studyName", "geno", paste(Trait))]
...
############################################################
# option 2- with alternate connecting genotypes 
############################################################
#Step 0

#The condition to be checked in this scenario
#1. A minimum of 60 common accessions required across trials for this scenario to work 
   # since we need 20 common accessions across trials and 20 trial to connect trial1 and Trial2, 
   # and 20 accessions to connect Trial2 and Trial 3

#So we need filter trials have more than 60 accessions common across trials 
Geno_WithNO_Na_sel <- tibble() # select genotypes with observed value across trials
for(j in colnames(Y1)){
  gn_withoutNa <- rownames(Y1)[which(!is.na(Y1[,j]))]
  gen1 <-  cbind(Trial = j, geno_name = length(gn_withoutNa))
  Geno_WithNO_Na_sel <- rbind(Geno_WithNO_Na_sel, gen1)
}
Geno_WithNO_Na_sel$geno_name <- as.numeric(Geno_WithNO_Na_sel$geno_name )
id <- which(Geno_WithNO_Na_sel$geno_name < 150)
if(length(id) > length(colnames(Y1))/2){
Y11 <- Y1[,id]
}else{
  Y11= Y1[, -id]
}
dim(Y11)
Acc_MegaLMM_scenario_4 <- tibble() 
for(pr in 1:10){

#####
# option 4 - With common genotype and alternate connecting genotypes across trials 

# set the testing set
Trial_smp <- sample(unique(colnames(Y11)),size = 3, replace = FALSE) # sample trials
#step1.

Y2 <- Y11[,Trial_smp] # subset sampl trial data
summary(Y2)
# set the testing set
...
#####
#Selecting accessions different from the common accessions across trial 1,2, and 3 
...
##Selecting the common accessions for Trial1 and Trial2, Trial2 and Trial3
 gencom_12 <- sample(intersect(Geno_Trial1_sel,Geno_Trial2_sel),size = 20, replace = F)
 gencom_23 <- sample(intersect(Geno_Trial2_sel[!Geno_Trial2_sel %in% gencom_12],Geno_Trial3_sel),size = 20, replace = F)

 Reduce(intersect,list(gencom_12,gencom_23,gen_intsel))

# Int123 <- Reduce(intersect, list(Geno_Trial1_sel, Geno_Trial2_sel, Geno_Trial3_sel)) #common across trials
# if(length(Int123) > 20 & length(Int123) < 30 ){
# gencom_12 <- sample(intersect(Geno_Trial1_sel,Geno_Trial2_sel[!Geno_Trial2_sel %in% Int123]),size = 20, replace = F)
# gencom_23 <- sample(intersect(Geno_Trial2_sel,Geno_Trial3_sel)[!intersect(Geno_Trial2_sel,Geno_Trial3_sel) %in% gencom_12] ,size = 20, replace = F)
# }else if(length(Int123) > 100){
#   gencom_12 <- sample(intersect(Geno_Trial1_sel,Geno_Trial2_sel),size = 20, replace = F)
#   gencom_23 <- sample(intersect(Geno_Trial2_sel,Geno_Trial3_sel)[!intersect(Geno_Trial2_sel,Geno_Trial3_sel) %in% gencom_12] ,size = 20, replace = F)
#   }else{
#   gencom_12 <- sample(intersect(Geno_Trial1_sel,Geno_Trial2_sel),size = length(Int123), replace = F)
#   gencom_23 <- sample(intersect(Geno_Trial2_sel,Geno_Trial3_sel)[!intersect(Geno_Trial2_sel,Geno_Trial3_sel) %in% gencom_12] ,size = 20, replace = F)
#   }
# 
# intersect(gencom_12,gencom_23)

Train_geno1 <- c(gen_intsel,gencom_12, sample(Geno_Trial1_sel[!Geno_Trial1_sel %in% c(gencom_12,gencom_23,gen_intsel)], size = 0))
Train_geno2 <- c(gen_intsel,gencom_12,gencom_23, sample(Geno_Trial2_sel[!Geno_Trial2_sel %in% c(gencom_12,gencom_23, Train_geno1, gen_intsel)], size = 0))
Train_geno3 <- c(gen_intsel,gencom_23, sample(Geno_Trial3_sel[!Geno_Trial3_sel %in% c(gencom_12,gencom_23, Train_geno1, Train_geno2, gen_intsel)], size = 0))
intersect(Train_geno1,Train_geno2)
...
Acc_MegaLMM_scenario_4 = rbind(Acc_MegaLMM_scenario_4, acc)

}
Acc_MegaLMM_scenario_4$Method<- "Scenario4"
Acc_MegaLMM_scenario_4$Group <- "Group6"
Acc_MegaLMM_scenario_4$plot_per_Trial <- 40
Acc_MegaLMM_scenario_4$Trait <- "Plant_height"
```
```{r plotting}

AccmegaLMMscen_comb <- rbind(Acc_MegaLMM_scenario_4,Acc_MegaLMM_scenario_3,Acc_MegaLMM_scenario_2,Acc_MegaLMM_scenario_1)

library(ggplot2)
AccmegaLMMscen_comb$Accuracy <- as.numeric(AccmegaLMMscen_comb$Accuracy)
ggplot(data = AccmegaLMMscen_comb, mapping = aes(x = Method, y = Accuracy )) + geom_boxplot(aes(fill = Method)) +  theme_bw()+
  theme( axis.text.x = element_blank()) 
write.csv(x = AccmegaLMMscen_comb, file = "output/Accuracy_MegaLMM_comb_group6_Plant_height_40.csv")
```
